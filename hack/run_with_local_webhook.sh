...
- admissionReviewVersions:
  ...
  clientConfig:
    caBundle: ${CA_BUNDLE}
    url: https://${CRC_IP}:${WEBHOOK_PORT}/validate-swift-openstack-org-v1beta1-swift
  failurePolicy: Fail
...
- admissionReviewVersions:
  ...
  clientConfig:
    caBundle: ${CA_BUNDLE}
    url: https://${CRC_IP}:${WEBHOOK_PORT}/mutate-swift-openstack-org-v1beta1-swift
  failurePolicy: Fail
  matchPolicy: Equivalent
  name: mswift.kb.io
  objectSelector: {}
...
CSV_NAME="$(oc get csv -n openstack-operators -l operators.coreos.com/swift-operator.openstack-operators -o name)"
...
if [ -n "${CSV_NAME}" ]; then
    ...
    oc patch "${CSV_NAME}" -n openstack-operators --type=json -p="[{'op': 'replace', 'path': '/spec/install/spec/deployments/0/spec/replicas', 'value': 0}]"
    oc scale --replicas=0 -n openstack-operators deploy/swift-operator-controller-manager
fi
