...
import (
	...
	"github.com/openstack-k8s-operators/swift-operator/internal/controller"
	webhookv1beta1 "github.com/openstack-k8s-operators/swift-operator/internal/webhook/v1beta1"
...
func main() {
	...
	if err := (&controller.SwiftProxyReconciler{
	...
	if err := (&controller.SwiftStorageReconciler{
		Client:  mgr.GetClient(),
		Scheme:  mgr.GetScheme(),
		Kclient: kclient,
	}).SetupWithManager(context.Background(), mgr); err != nil {
	...
	if err := (&controller.SwiftRingReconciler{
		Client:  mgr.GetClient(),
		Scheme:  mgr.GetScheme(),
		Kclient: kclient,
	...
	if err := (&controller.SwiftReconciler{
		Client:  mgr.GetClient(),
		Scheme:  mgr.GetScheme(),
		Kclient: kclient,
	}).SetupWithManager(mgr); err != nil {
	...
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err := webhookv1beta1.SetupSwiftWebhookWithManager(mgr); err != nil {
