...
# openstack.org/swift-operator-bundle:$VERSION and openstack.org/swift-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= quay.io/$(USER)/swift-operator
...
# The image tag given to the resulting catalog image (e.g. make catalog-build CATALOG_IMG=example.com/operator-index:v0.2.0).
...
force-bump: ## Force bump operator and lib-common dependencies
	...
	for dep in $$(cat api/go.mod | grep openstack-k8s-operators | grep -vE -- 'indirect|swift-operator|^replace' | awk '{print $$1}'); do \
